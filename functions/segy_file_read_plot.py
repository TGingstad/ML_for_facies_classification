...
def segy_file(file_name='', color_map='seismic', image=True):
    ...
    if image:
        ...
        if color_map =='seismic':
            ...
            facies_name = ['', 'Sand', 'Fine Sand','Coarse Sand', 'Shale', 'Carbonate']
            facies_color = ['#FFFFFF' , '#FFFF00', '#FFCC00', '#E69076', '#A6A6A6', '#8080FF']
        ...
        elif color_map == 'facies_simple':
            # Customise color map for facies
            facies_name = ['', 'Sand','Shale', 'Carbonate']
            facies_color = ['#FFFFFF' ,'#FFFF00','#A6A6A6', '#8080FF']
            cmap = matplotlib.colors.ListedColormap(facies_color)
            bounds = [-1.5, -0.5, 2.5, 3.5, 4.5]
            norm = matplotlib.colors.BoundaryNorm(bounds, cmap.N)
            im = ax.imshow(data_file.T, cmap=cmap, aspect='auto', extent=extent, norm=norm, vmin=0-1.5, vmax=4+0.5)
            fig.colorbar(im, ticks=np.arange(0, 5))
